...
class GroundedScanEnvironment(gym.Env):
    ...
    def __init__(self, path_to_data: str, save_directory: str, k: int, split="train", input_vocabulary_file="",
        ...
        # Observation space command
        depth_dimension = max(self.grid_dimensions[2], self.dataset.max_input_length)
        self.depth_dimension = depth_dimension
        observation_space_combined = spaces.Box(low=0, high=self.input_vocabulary_size - 1,
                                                shape=[self.grid_dimensions[0] + 1,
                                                       self.grid_dimensions[1],
                                                       depth_dimension], dtype=np.uint8)
        self.observation_space = observation_space_combined
        self._random_permutation = [i for i in range(len(self.dataset._data_pairs[self.split]))]

        self.evaluate_on_split = "dev"
        self.num_examples_dev = 1
    ...
    def combine_state(self, language_array, world_observation):
        language_tiled = np.tile(language_array, (1, self.grid_dimensions[0], 1))
        observation = np.concatenate((world_observation, language_tiled), axis=0)
        return observation
    ...
    def reset(self) -> np.ndarray:
        ...
        self._current_example_counter = self._current_example_counter % 1  # TODO: change
        ...
        language_observation, world_observation = self.dataset.initialize_rl_example(current_example)
        language_array = self.sentence_to_array(language_observation, vocabulary="input",
                                                pad_to=self.depth_dimension)
        self.current_language_observation = language_array
        if self._current_example_counter == 1 - 1:
            self.read_full_training_set += 1
            # self.shuffle_data()
        self._current_example_counter += 1
        return self.combine_state(language_array, world_observation)
    ...
    def percentage_progress(self) -> float:
        return (self._current_example_counter / 100) * 100

    def step(self, action: int) -> Tuple[np.ndarray, float, bool, dict]:
        action = action + 3  # TODO: check this
        if action == self.target_vocabulary.pad_idx or action == self.target_vocabulary.eos_idx \
            ...
            observation = self.combine_state(self.current_language_observation,
                                             self.dataset.get_current_situation_grid_repr())
        ...
        new_observation = self.combine_state(self.current_language_observation, new_situation)
